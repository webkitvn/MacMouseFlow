...
- [x] 2.2 Define the manually maintained C header and matching primitive fixed-width `#[repr(C)]` POD types, opaque engine pointer, `_v1` declarations, status values, layout rules, and caller ownership rules; verified Rust layout/contract tests and C compilation consume the checked-in header. The owner uniquely destroys; non-owning copies may update/evaluate concurrently while owner-retained lifetime holds.
- [x] 2.3 Implement minimal safe conversion and export code with exact layout validation, `catch_unwind`, output-first Preserve initialization, configuration delegation, and pointer-to-handle destruction that nulls before free; verified all Rust FFI contract tests pass (`cargo test -p pointer-input-ffi`) and evaluation failure preserves output.
