...
The FFI crate owns a single process-global panic hook. Creation uses synchronized uninstalled/installing/installed state: a create invoked from a panicking thread while uninstalled returns the public `Panic` status with a null owner before changing state, and a later normal create may install; one creator otherwise installs outside the installation-state lock before allocating an engine or allowing native input delivery; every additional creator during installation returns the public `Busy` status with a null owner without waiting; the bounded integration helper automatically retries only `Busy`, and a caller that received `Busy` may retry after the installer succeeds or fails; installation failure returns the public `Panic` status and resets state to uninstalled, after which a caller may make a separate later creation; and success publishes installed exactly once for the process lifetime. It captures the prior hook and intentionally leaves the installed policy in place through ordinary engine destruction.
