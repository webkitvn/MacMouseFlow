...
A failed local install or update SHALL not replace the known-good artifact and SHALL not unexpectedly change runtime configuration. Before mutating an existing active copy in any lifecycle action — install, rollback, or uninstall alike — the path SHALL validate its bundle ID, expected executable, ad-hoc signature, and launch probe; any foreign or malformed active copy, or unexpected or malformed rollback content, SHALL abort without mutation. Lifecycle tooling SHALL own only `LocalShip/` and SHALL not read, parse, migrate, rewrite, delete, or repair runtime configuration. It SHALL prove configuration unchanged by existence, byte length, and SHA-256 before and after; when real configuration is absent, it SHALL use an opaque sentinel outside `LocalShip/` in app-owned Application Support and prove it byte-identical. Every lifecycle action SHALL acquire a nonblocking native advisory lock and create one atomic pipeline-owned lifecycle record before mutation, containing the operation and bounded survivor identity. Valid operations are exactly `install`, `rollback`, and `uninstall`; a non-null survivor identity is valid only when it names an existing allowed pipeline-owned terminal-survivor location. A malformed, stale, or out-of-scope record SHALL abort later automation without mutation or cleanup and without reporting a survivor until an operator resolves it; a fully successful action SHALL remove the record. The local pipeline SHALL not add an updater or installer subsystem, process, helper, daemon, runtime ABI change, or input behavior change. Its persistent lifecycle state is exactly one canonical rollback bundle, the bounded terminal recovery bundle below, and the in-flight lifecycle record; it SHALL not add further state stores. It SHALL not add Intel `x86_64` acceptance, Rosetta 2 compatibility handling, Universal Binary packaging, or runtime CPU-architecture detection; the project supports macOS 14+ on Apple Silicon (`arm64`) only through v1 (ADR-0006).
...
#### Scenario: Lifecycle record blocks interrupted automation
- **WHEN** a subsequent `install`, `rollback`, or `uninstall` finds a malformed or retained lifecycle record
- **THEN** it refuses before mutation or cleanup, reports explicit operator resolution is required, and reports the recorded survivor identity only when present, allowed, and existing; stale, out-of-scope, or malformed identities are not reported as survivors.

#### Scenario: Concurrent lifecycle action refuses
- **WHEN** another lifecycle action holds the advisory lock
- **THEN** the new action returns structured `lifecycle already running` without mutation

#### Scenario: Uninstall removal fails
- **WHEN** recursive removal of the active bundle raises an operating-system error
- **THEN** uninstall stops without cleanup, verifies configuration evidence, and returns the observed active, rollback, and recovery state without a preservation claim

#### Scenario: Automated lifecycle action refuses while a terminal BLOCKED state is outstanding
- **WHEN** a subsequent `install`, `rollback`, or `uninstall` finds a retained lifecycle record from a prior true triple filesystem failure
- **THEN** the lifecycle action refuses before mutation, cleanup, or survivor-path inference, and returns a structured failure requiring operator resolution. If updating the record with a preserved triple-failure survivor fails, the immediate failure still reports that exact survivor, while the retained pre-mutation record blocks later automation without a survivor claim.
